...
### โปรแกรมอัพเดทยังไง?
เมื่อมีเวอร์ชั่นใหม่:
1. เปิดโปรแกรม → 5 วินาที → เช็คอัพเดทอัตโนมัติ
2. ปุ่ม **"🆕 New Update"** สีแดงกระพริบจะปรากฏขวาสุด topbar
3. กดปุ่ม → ดู changelog → กด "อัพเดทตอนนี้"
4. ดาวน์โหลด patch (11-63 MB) → ปิดโปรแกรม → แตกไฟล์ → เปิดใหม่อัตโนมัติ

### ทำไมไม่เห็นปุ่ม New Update?
- ถ้าเวอร์ชั่นเท่ากัน → ไม่แสดง (ปกติ)
- ถ้าเช็คไม่ได้ (network/SSL) → ลองไปที่ **ตั้งค่า > เกี่ยวกับ > เช็คอัพเดท**
- ถ้าอัพเดทไม่สำเร็จ → ปุ่มจะขึ้นอีกครั้งเมื่อเปิดโปรแกรมใหม่

### จะมี loop update ไหม?
ไม่ — หลังอัพเดท version.json ใหม่จะถูกแตกทับ → เปิดใหม่ → version เท่ากัน → ไม่เด้งปุ่ม

---

## 💚 ระบบ Supporters (ผู้สนับสนุน)

### ส่งหลักฐานสนับสนุนยังไง?
1. ในโปรแกรม: ตั้งค่า > สนับสนุน > กด "แนบหลักฐานสนับสนุน"
2. เลือกช่องทาง: ธนาคาร (แนบสลิป หรือ กรอกข้อมูล) / True Money
3. กรอกชื่อ + จำนวน + (optional) แพลตฟอร์ม + ลิงก์ช่อง
4. ส่ง → admin จะได้รับแจ้งใน Discord → approve → ชื่อขึ้นในโปรแกรม
...
### โค้ดลับคืออะไร?
viewer พิมพ์โค้ด (เช่น `!wow`) ในแชท → โปรแกรมตัดโค้ดออก → TTS อ่านส่วนที่เหลือ → เล่นเสียงที่ตั้งไว้

### ตั้งค่ายังไง?
1. ตั้งค่า > โค้ดลับ
2. กรอกโค้ด (เช่น `wow` → จะกลายเป็น `!wow`)
3. กด "เพิ่ม" → เลือกไฟล์เสียง (.mp3/.wav)
4. ปรับ volume + ทดลองฟัง (Preview/Stop)
5. ตั้งจำกัด/user/วัน (ถ้าต้องการ)
